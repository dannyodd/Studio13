...
//Check for POST data from the form in add_new_product.php - If not received, handle errors
if (!filter_has_var(INPUT_POST, 'product_name') ||
    ...
    echo "Error validating form POST data. Product insert canceled, please check input values.";
...
//Construct MySQL Insert statement
$query_str = "INSERT INTO products VALUES (NULL, '$product_name', '$artist_id', '$product_price', '$product_description', '$product_image_link')";
...
if (!$query) {
    $errno = $conn->errno;
    $errmsg = $conn->error;
    echo "Insertion failed with: ($errno) $errmsg.";
    $conn->close();
    include 'includes/footer.php';
    exit;
}

//Set the product_ID of the new product and close the connection
